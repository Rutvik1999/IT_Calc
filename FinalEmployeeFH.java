        ...
        do
        {
        	...
        	try
        	{
				...
				result1_label: while(true)
				{
					...
					try
					{
						int result1 = JOptionPane.showConfirmDialog(null,fields1,"Personal Details",JOptionPane.OK_CANCEL_OPTION);
						...
						else if(result1 == JOptionPane.OK_OPTION)
						{
							...
							{
								if(Character.isLetter(name.getText().charAt(i)) || (name.getText().charAt(i) == ' ' && name.getText().charAt(i+1) != ' '))
									set_NAME = true;
								else 
								{
									set_NAME = false;
							...
							for (int i = 0; i < 10; i++) 
							{
								...
								else if ((i >= 5 && i != 9) && (int)PAN.charAt(i) <= 57 && (int)PAN.charAt(i) >= 48)
								{	
									set_PAN2 = true;
								}
							...
							if(email.getText().contains("@") && (email.getText().contains(".c") || email.getText().contains(".in")))
					...
					{
						int temp_r1 = JOptionPane.showConfirmDialog(null, "Please Enter Date in Appropriate format\nNOTE: Age should be greater than 18\nto be elegible to pay tax","Invalid Date", JOptionPane.OK_OPTION);
					...
					{
						int temp_r1 = JOptionPane.showConfirmDialog(null,"Please Enter Appropriate Data","Invalid Data", JOptionPane.OK_OPTION);
						if(temp_r1 == -1 || temp_r1 == 1)
						{
							JOptionPane.showMessageDialog(null, "Exiting Program");
							System.exit(-1);
				...
				city_type = JOptionPane.showOptionDialog(null,"Select type of City (of Residence)", "Type of City", JOptionPane.DEFAULT_OPTION, JOptionPane.INFORMATION_MESSAGE, null, option, option[1]);
				if (city_type == -1) 
				{
					System.exit(-1);
				}
				...
				result2_label: while(true)
				{
					...
					{
						int temp_r2 = JOptionPane.showConfirmDialog(null,"Please Enter Appropriate Data","Invalid Data", JOptionPane.OK_OPTION);
				...
				result4_label: while(true)
				{
					...
					{
						int temp_r4 = JOptionPane.showConfirmDialog(null,"Please Enter Appropriate Data","Invalid Data", JOptionPane.OK_OPTION);
				...
				result5_label: while(true)
				{
					try
					{		
						int result5 = JOptionPane.showConfirmDialog(null,fields5,"Deduction Under Chapter VI-A",JOptionPane.OK_CANCEL_OPTION);
					...
					{
						int temp_r5 = JOptionPane.showConfirmDialog(null,"Please Enter Appropriate Data","Invalid Data", JOptionPane.OK_OPTION);
				...
				result5a_label: while (true) 
				{
					...
					try 
					{
						int result5a = JOptionPane.showConfirmDialog(null, fields5a, "Advance Tax", JOptionPane.OK_CANCEL_OPTION);
					...
					{
						int temp_r5a = JOptionPane.showConfirmDialog(null, "Please Enter Appropriate Data","Invalid Data", JOptionPane.OK_OPTION);
				...
				CEmployee5 emp = new CEmployee5(arr_Str[0], arr_Str[1], age, arr_Str[2], arr[0], city_type, arr[1], arr[2], arr[3], arr[4], arr[5], arr[6], arr[7], arr[8], arr[9], arr[10], arr[11], arr[12], arr[13], arr[14], arr[15], arr[16], arr[17], arr[18], arr[19], arr[20], arr[21], arr[22]);
		        
		        JOptionPane.showMessageDialog(null,"Tax Rebate of Rs. 2,500/- (For Income of less than 3.5 Lakhs)"+"\nTax Surcharge @ 10% (For Income of more than 50 Lakhs)"+"\nEducation Cess + Health Cess @ 4%\n\n" + "Total Taxable Income : " + emp.getTotIncome() + "\nTotal Tax Payable        : " + emp.getTotTaxPayable() + "\nNet Tax Payable           : " + emp.getNetTaxPayable_Remaining());
		        ...
		        int x = JOptionPane.showConfirmDialog(null,"The file has been created.\nPressing the Yes button will open the file.", "File",JOptionPane.OK_OPTION);
